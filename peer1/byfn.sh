...
function networkUp() {
  ...
  if [ "${CERTIFICATE_AUTHORITIES}" == "true" ]; then
    ...
    export BYFN_CA1_PRIVATE_KEY=$(cd crypto-config/peerOrganizations/org1.weshare.com/ca && ls *_sk)
  fi
...
function upgradeNetwork() {
  if [[ "$IMAGETAG" == *"1.4"* ]] || [[ $IMAGETAG == "latest" ]]; then
    docker inspect -f '{{.Config.Volumes}}' orderer.weshare.com | grep -q '/var/hyperledger/production/orderer'
    if [ $? -ne 0 ]; then
    ...
    docker-compose $COMPOSE_FILES stop orderer.weshare.com
    docker cp -a orderer.weshare.com:/var/hyperledger/production/orderer $LEDGERS_BACKUP/orderer.weshare.com
    docker-compose $COMPOSE_FILES up -d --no-deps orderer.weshare.com

    for PEER in peer0.org1.weshare.com peer1.org1.weshare.com; do
...
function replacePrivateKey() {
  ...
  CURRENT_DIR=$PWD
  cd crypto-config/peerOrganizations/org1.weshare.com/ca/
  PRIV_KEY=$(ls *_sk)
  cd "$CURRENT_DIR"
...
# (``peer0.org1.weshare.com`` & ``peer0.org2.weshare.com``).  Secondly, we point to
...
COMPOSE_FILE=docker-compose-peer1.yaml
#
